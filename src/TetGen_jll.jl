...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import TetGen_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("TetGen_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "TetGen_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"TetGen", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "TetGen_jll", "src", "TetGen_jll.jl"))
        @info("TetGen_ll dev'ed out to /home/fuhrmann/Wias/work/julia/dev/TetGen_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("b47fdcd6-d2c1-58e9-bbba-c1cee8d8c179"))
platforms = [Pkg.Artifacts.unpack_platform(e, "TetGen", artifacts_toml) for e in artifacts["TetGen"]]
...
if best_platform === nothing
    @debug("Unable to load TetGen; unsupported platform $(triplet(platform_key_abi()))")
else
